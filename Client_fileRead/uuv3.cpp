...
void UUV3::connectionUUV3(unsigned short port, QString ip)
{
    ...
    // 发送一个信号告知主线程：客户端已经成功连接到了UUV3服务端（信号也可以作为槽函数!!!）
    connect(m_tcp, &QTcpSocket::connected, this, &UUV3::connectUUV3_OK);
...
// 负责向UUV3服务端发送相关指令的任务函数
void UUV3::queryPkgNums()
{
    QString query = "queryPkgNums";
    m_tcp->write(query.toUtf8()); // 向客户端发送一条查询指令
}

void UUV3::readReadyOK(int startPkg, int endPkg)
{
    m_startPkg = startPkg;
    m_endPkg = endPkg;
    idxPkg = startPkg - 1;
